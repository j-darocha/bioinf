#This python library was created by Dr. H Othman -  https://github.com/hothman
...
def RMSD(trajectory, reference, adjust_time = 0.01):
    ...
    backbone_ref = reference.top.select('backbone') 
...
def RMSD_select(trajectory, reference, adjust_time = 0.01):
    """
    Calculate RMSD
    """
    trajectory.center_coordinates()
    backbone = trajectory.top.select('(resid 1 to 200) and backbone') 
    backbone_ref = reference.top.select('(resid 1 to 200) and backbone') 
    #not (resid 234 to 243 or resid 254 to 262 )  and name CA
    rmsd = md.rmsd(trajectory, reference, 0, atom_indices=backbone, ref_atom_indices=backbone_ref )
    time = np.arange(len(rmsd))
    print(backbone)
...
def RMSD_selectAS(trajectory, reference, adjust_time = 0.01):
    """
    Calculate RMSD
    """
    trajectory.center_coordinates()
    backbone = trajectory.top.select('(resid 200 to 206) and backbone') 
    backbone_ref = reference.top.select('(resid 200 to 206) and backbone') 
    #not (resid 234 to 243 or resid 254 to 262 )  and name CA
    rmsd = md.rmsd(trajectory, reference, 0, atom_indices=backbone, ref_atom_indices=backbone_ref )
    time = np.arange(len(rmsd))
    print(backbone)
...
def plotPcs(list_pcs, row_number, col_number, xmin , xmax, ymin,ymax, index_of_wt = 0, pc1 = 1, pc2=2, point_size=5  ): 
    ...
    colormap_var = [(232, 19, 15), (232, 150, 150)]  # dark red to light red 
    colormap_wt = [(100, 100, 100),(200, 200, 200)]  # dark grey to light grey
